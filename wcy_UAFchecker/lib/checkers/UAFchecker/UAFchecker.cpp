...
void immediate_uaf_checker::cehck() {
	lableAllFreeMethod();	//清空，初始化	
    for(auto F:resource->getFunctions()) {
		FunctionDecl *FD = manager->getFunctionDecl(F);
		std::unique_ptr<CFG> &function_cfg = manager->getCFG(F);       //control flow graph 控制流程图
		...
		for(CFG::iterator cfgiter=cfg->begin();cfgiter!=cfg->end();++cfgiter){		//遍历
			CFGBlock* block=*cfgiter;
			for(CFGBlock::iterator blockiter=block->begin();blockiter!=block->end();blockiter++)  //遍历控制块
			{
				CFGElement element=*(blockiter);
				if(element.getKind()==CFGElement::Kind::Statement)
				{
					const Stmt* stmt=((CFGStmt*)&element)->getStmt(); //获取语句  //if for while 普通
					const FunctionDecl *callee = nullptr;
					std::unordered_set<const Expr*> freeargs;  //free 的参数
					bool uafFlag=false;
					if (const CXXDeleteExpr* deletexpr = dyn_cast<CXXDeleteExpr>(stmt)) { //delete 语句  free C语句  C++兼容  delete 只有C++支持 C不支持
					...
					if (freeargs.empty()) continue;  //没有找到 free的地方 继续下一个循环，找到了则判断
					CFGBlock::iterator tmpitr = blockiter+1;
					while(tmpitr != block->end()){
						CFGElement element2=*(tmpitr);
						const Stmt* stmt2=((CFGStmt*)&element2)->getStmt();		//使用了 freeargs 的语句					
						if (isFreedArgsReferenced(stmt2,freeargs)){	 //检测 use after free  AST  根据树的枝叶关系，层级关系去判断的
					...
					if (!uafFlag) {
						...
						checkSuccessor(stmt,block,freeargs,SM,checkDepth);  //如果没有 到达uaf，进行下一层代码的check 
					}					
				}
			}
		}		
    ...
    for(CFGBlock::succ_iterator succ_iter=block->succ_begin();succ_iter!=block->succ_end();++succ_iter){
        ...
        for(CFGBlock::iterator succblockiter=succblock->begin();succblockiter!=succblock->end();succblockiter++) {
            ...
            if(element3.getKind()==CFGElement::Kind::Statement)
            {
                ...
                if (isFreedArgsReferenced(stmt3,freeargs)){ //检测 use after free  AST  根据树的枝叶关系，层级关系去判断的
...
bool immediate_uaf_checker::isFreedArgsReferenced(const Stmt* stmt, std::unordered_set<const Expr*>& freeargs) {
    ...
    for (auto e:freeargs) {
		...
		if (stmts.find(vs) != std::string::npos) {
			...
			visitor.TraverseStmt((Stmt*)stmt);		//调用Clang 的接口   RecursiveASTVisitor 基于 FindUAFVarDeclRef 方法去判断  clang 源码
			if (visitor.isVarDeclUsed(freevar)) {		// use after free is true
				return true;
			}
		}
	}
...
void immediate_uaf_checker::lableAllFreeMethod() {
	for(auto F:resource->getFunctions()) {	//遍历所有函数  AST
		finder->clearFreeCalls(); //清空，初始化
		checkFreeMethod(F);	 //检查有没有调用free/delete 接口  free(xx) delete a;
	}
}

void immediate_uaf_checker::checkFreeMethod(ASTFunction* astF) {
	std::vector<bool> parms;   //STL 标准库  vector   ===  C 数组   
	FunctionDecl* FD = manager->getFunctionDecl(astF);  //clang FunctionDecl 
	if (!FD) return;	//如果为空则返回 // LLVM   AST

	std::string funcName =  FD->getNameInfo().getName().getAsString();  // 返回当前函数调用的名称 返回csting 类型
	if (funcName == "free") {
		common::printLog(FD->getNameAsString()+" is free method\n",common::CheckerName::immediate_uaf_checker,2,*configure);	//打印一串log
		//parms.push_back(true);
		//finder->updateFreeMethods(FD,parms);
		return;
	}
	
	for (int i=0;i<FD->getNumParams();i++){ //记录函数的传参   char* ar =  malloc(7)   delete ar;
 		ParmVarDecl* p = FD->getParamDecl(i);
		parms.push_back(false);

	}
	//FindCalledExpr finder;
	if (!FD->getBody()) return;  // 如果没有函数体，则返回
	finder->Visit(FD->getBody());	// 如果存在函数体，则访问函数体	//形成一个结构体
	bool flag = false;
	std::vector<const CallExpr*> freeCalls = finder->getFreeCalls();	//获取所有调用free 语句的列表
	for(auto c : freeCalls) {	//
		...
		for (int i=0;i<c->getNumArgs();i++){
			...
			if (p){
				int pos = getParameterPosition(FD,p);  //记录位置
				if (pos == -1) continue;	//退出当前循环
				parms[pos]=true;	
				flag = true;
	...
	if (flag){
		finder->updateFreeMethods(FD,parms);  //找到符合条件的，插入列表
	}
}

int immediate_uaf_checker::getParameterPosition(FunctionDecl* FD, const ParmVarDecl* p){
	for (int i=0;i<FD->getNumParams();i++){
		ParmVarDecl* pp = FD->getParamDecl(i);	//参数在参数表(数组)的下标位置
		if (p == pp) return i;
	}
	return -1;
}

const VarDecl* immediate_uaf_checker::getVarDecl(const Expr* e) {  //获取引用
